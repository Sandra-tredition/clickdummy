...
const EditionDetailPage = () => {
  ...
  // Generate edition label for breadcrumb
  const getEditionLabel = () => {
    if (!edition) return "Neue Ausgabe";

    let label = edition.produktform || "Ausgabe";
    if (edition.ausgabenart) {
      label += ` - ${edition.ausgabenart}`;
    }
    return label;
  };
  ...
  const breadcrumbs = [
    ...
    { label: "Mein Buch", href: "/project/1" }, // Mock project name and link
    { label: getEditionLabel() },
  ];

  return (
    <Layout title={edition?.title || "Neue Ausgabe"} breadcrumbs={breadcrumbs}>
